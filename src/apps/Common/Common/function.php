...
function get_nickname($uid = 0) {
    ...
    if (isset($list[$key])) { //已缓存，直接使用
        ...
        $info = M('MemberProfile')->field('nickname')->find($uid);
        if ($info !== false) {
            $nickname = empty($info['nickname']) ? get_mobileByUid($uid) : $info['nickname']; //未设定昵称的指定用户手机
 ...
 * @return string       用户昵称
 */
function get_mobileByUid($uid = 0) {
    static $list;
    if (!($uid && is_numeric($uid))) { //获取当前登录用户名
        return session('user_auth.username');
    }
    ...
    if (isset($list[$key])) { //已缓存，直接使用
        ...
        $mobile = '';
        $rootid = M('MemberAccount')->where("uid={$uid}")->getField('rootid');
        if ($rootid) {
            $mobile = M('Member')->where("id={$rootid}")->getField('mobile');
        }
        if ($mobile) {
            $list[$key] = $mobile;
            /* 缓存用户 */
 ...
 * @return uids
 */
function getMemberLikeInfo($nickname = ""){
    $ids = "";
    if($nickname != ""){
        $nmap['nickname'] = array('like', '%' . $nickname . '%');
        $mmap['mobile'] = array('like', '%' . $nickname . '%');
        $nArray = M('memberProfile')->where($nmap)->select();
        $mArray = M('member')->where($mmap)->select();
        $mds = "";
        foreach ($mArray as $key => $val) {
            $mds = $mds . $val['id'] . ",";
        }
        $amap['rootid'] = array('in', $mds); 
        $aArray = M('memberAccount')->where($amap)->select();
...
function get_album($cover_id){
    if(empty($cover_id)){
        return false;
    }
//    dump($cover_id);exit();
    $picture = M('product_album')->where("img_id = {$cover_id}")->getField('img_url');
//    dump('sss');exit();
    return $picture;
}
...
function rrmdir($dir) { 
       ...
       * 发送短信
       */
      function send_short_msg($mobile = '', $message = ''){
          $sms = D('Common/Sms', 'Logic');
            $rst = $sms->send($mobile, $message);
            return $rst;
      }
       ...
       * 发送邮件
       */
      function send_email($title = '', $content = '', $to_email = ''){
          $email = D('Common/Email', 'Logic');
        $rst = $email->sendmail($to_email,$title,$content);
        return $rst;
      }
