...
function getUserAvatar($uid) {
    $avatar = S('U_AVATAR_' . $uid);
    if (!$avatar) {
        $profile = S('U_PROFILE_' . $uid);
        if (!$profile) {
            $profile = M('MemberProfile')->find($uid);
            S('U_PROFILE_' . $uid, $profile);

            $avatar = empty($profile['headimg']) ? C('DEFAULT_USER_AVATAR') : $profile['headimg'];
            S('U_AVATAR_' . $uid, $avatar);
        }
    }
    return $avatar;
}
