...
//         	if ($ios) {
    ...
    function sendAndroidCustomizedcast($data) {
    	...
    	if(is_array($data['uid'])){
    	    foreach($data['uid'] as $value) {
    	        ...
    	        if($alias)
    	            $alias .= ','. md5($value.'NIAOTING');
    	        else 
    	            $alias .= md5($value . 'NIAOTING');
    	    }
    	} else {
    	    $alias = md5($data['uid'].'NIAOTING');
    	}
    ...
    function sendIOSCustomizedcast($data) {
		...
		if (isset($data['after_open']['type'])) {
			...
			if (isset($data['extra']['goTypeID'])) $arr['goTypeID'] = $data['extra']['goTypeID'];
                        if (isset($data['extra']['title'])) $arr['title'] = $data['extra']['title'];
			
			$customizedcast->setPredefinedKeyValue('niaoting',		$arr);
		}
		
		$customizedcast->setPredefinedKeyValue('production_mode',	$this->productionMode);
		
		if (isset($data['text'])) {
			$customizedcast->setPredefinedKeyValue('description',	$data['text']);
		}
    	return $customizedcast->send();
    }
    ...
    function sendIOSBroadcast($data) {
    	...
    	if (isset($data['after_open']['type'])) {
			...
			$brocast->setPredefinedKeyValue('niaoting',		$arr);
		}
		
		$brocast->setPredefinedKeyValue('production_mode',	$this->productionMode);
		
		if (isset($data['text'])) {
			$brocast->setPredefinedKeyValue('description',	$data['text']);
		}
