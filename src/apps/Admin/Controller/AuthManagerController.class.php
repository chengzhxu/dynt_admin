...
class AuthManagerController extends AdminController
{
    ...
    {
        $list = $this->lists('SysAuthGroup', array('module' => 'admin'), 'id asc');
    ...
    {
        $auth_group = M('SysAuthGroup')->where(array('module' => 'admin', 'type' => AuthGroupModel::TYPE_ADMIN))
            ->find((int)$_GET['id']);
    ...
    public function changeStatus($method = null)
    {
        ...
        switch (strtolower($method)) {
            case 'forbidgroup':
                $this->forbid('SysAuthGroup');
                break;
            case 'resumegroup':
                $this->resume('SysAuthGroup');
                break;
            case 'deletegroup':
                $this->delete('SysAuthGroup');
                break;
            default:
    ...
    public function user($group_id)
    {
        ...
        $auth_group = M('SysAuthGroup')->where(array('status' => array('egt', '0'), 'module' => 'admin', 'type' => AuthGroupModel::TYPE_ADMIN))
        ...
        $_REQUEST = array();
        $list = $this->lists($model, array('a.group_id' => $group_id), 'm.uid asc', null, 'm.*');
        int_to_string($list);
        $this->assign('_list', $list);
        $this->assign('auth_group', $auth_group);
    ...
    {
        $uid = I('uid');
        $auth_groups = D('SysAuthGroup')->getGroups();
    ...
    public function deleteNode(){
        $aId=I('id',0,'intval');
        if($aId>0){
           $result=   M('SysAuthRule')->where(array('id'=>$aId))->delete();
            if($result){
                $this->success('删除成功。');
            }else{
                $this->error('删除失败。');
    ...
    public function access()
    {
        ...
        $main_rules = M('SysAuthRule')->where($map)->getField('name,id');
        $map = array('module' => 'admin', 'type' => AuthRuleModel::RULE_URL, 'status' => 1);
        $child_rules = M('SysAuthRule')->where($map)->getField('name,id');
    ...
    public function accessUser()
    {
        ...
        $this->updateRules();
        $auth_group = M('SysAuthGroup')->where(array('status' => array('egt', '0'), 'type' => AuthGroupModel::TYPE_ADMIN))
            ->getfield('id,id,title,rules');
        $node_list = $this->getNodeListFromModule(D('Common/Module')->getAll());

        //  $node_list   =M('SysAuthRule')->where(array('module'=>array('neq','admin'),'type'=>AuthRuleModel::RULE_URL,'status'=>1))->select();
    ...
    private function getMergedRules($oldRules, $rules, $isAdmin = 'neq')
    {
        ...
        $otherRules = M('SysAuthRule')->where($map)->field('id')->select();
    ...
    public function getNodeListFromModule($modules)
    {
        ...
        foreach ($modules as $module) {
            if ($module['is_setup']) {
                ...
                $node['child'] = M('SysAuthRule')->where($map)->select();
                $node_list[] = $node;
            }
