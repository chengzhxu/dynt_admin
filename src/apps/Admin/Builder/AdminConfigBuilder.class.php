...
class AdminConfigBuilder extends AdminBuilder
{
    ...
    public function keyEditor($name, $title, $subtitle = null, $config = '', $style = array('width' => '500px', 'height' => '400px'))
    {
        ...
        if (empty($config)) {
            $toolbars = "toolbars:[['source','|','bold','italic','underline','fontsize','forecolor','justifyleft','justifycenter','justifyright','fontfamily','|','map','emotion','insertimage','insertcode']]";
        }
        if ($config == 'all') {
            $toolbars = 'all';
        }
    ...
    public function keyOssImage($name, $title, $subtitle = null)
    {
        return $this->key($name, $title, $subtitle, 'ossImage');
    }
    ...
    public function buttonSubmit($url = '', $title = '确定' ,$name='')
    {
        if ($url == '') {
            $url = __SELF__;
        }
