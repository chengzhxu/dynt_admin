...
class Wechat
{
	...
	const DEVICE_GET_STAT                 = '/device/get_stat?';        //设备状态查询
	const DEVICE_GET_OPENID               = '/device/get_openid?';      //获取设备绑定openID
	const DEVICE_GET_BIND_DEVICE          = '/device/get_bind_device?'; //获取用户绑定的deviceid
	const DEVICE_BIND                     = '/device/bind?';            //获取设备二维码
	const DEVICE_UNBIND                   = '/device/unbind?';          //获取设备二维码
	const DEVICE_VERIFY_QRCODE            = '/device/verify_qrcode?';   //验证二维码
	///数据分析接口
...
// 		$this->log($postStr);
	...
	*/
	 ...
	 * @param string $mediaid
	 */
	public function deviceMsg($DeviceType='' , $DeviceID = '', $SessionID = '' , $Content = '')
	{
		$msg = array(
			'ToUserName' => $this->getRevFrom(),
			'FromUserName'=>$this->getRevTo(),
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /**
     * 获取设备二维码
     * @param String $device_id 要获取二维码设备的 device_id
     */
    function createQrcode($device_id = '') {
    ...
    /**
     * 获取设备状态
     * @param String $device_id 要获取设备状态的 device_id
     */
	...
	/**
     * 验证设备二维码
     * @param String $ticket 设备二维码的ticket,可能过createQrcode获取到
     */
	function verifyQrcode($ticket = '') {
		...
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::DEVICE_VERIFY_QRCODE . 'access_token=' . $this->access_token , self::json_encode($data));
	...
	/**
     * 获取设备绑定openID
     * @param String $device_id 要获取设备openid的 device_id
     */
	function getOpenid($device_id = '' , $device_type = '') {
		...
		$result = $this->http_get(self::API_BASE_URL_PREFIX . self::DEVICE_GET_OPENID . 'access_token=' . $this->access_token . '&device_type=' . $device_type .'&device_id=' . $device_id);
     ...
     * @param String $openid 当前登录用户的openid
     */
	function getBindDevice($openid) {
		...
		$result = $this->http_get(self::API_BASE_URL_PREFIX . self::DEVICE_GET_BIND_DEVICE . 'access_token=' . $this->access_token . '&openid=' . $openid);
	 ...
	 * @param string $ticket 
	 */
	function bind($ticket = '' , $device_id = '' , $openid = '') {
	    
	    if (!$this->access_token && !$this->checkAuth()) return false;
	    
	    if(!$openid || !$ticket ||!$device_id)
	        return false;
		...
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::DEVICE_BIND . 'access_token=' . $this->access_token , self::json_encode($data));
	    print_r($result);print_r($data);exit;
	    if($result) {
	        $json = json_decode($result, true);
	        if (!$json || !empty($json['errcode'])) {
	            $this->errCode = $json['errcode'];
	...
	/**
	 * 设备解绑
	 * @param string $ticket 绑定操作合法性的凭证（由微信后台生成，第三方H5通过客户端jsapi获得）
	 * @param string $device_id 设备id 
	 * @param string $openid 用户对应的openid 
	 */
	function unbind($ticket = '' , $device_id = '' , $openid = '') {
	    ...
	    $result = $this->http_post(self::API_BASE_URL_PREFIX . self::DEVICE_UNBIND . 'access_token=' . $this->access_token  , self::json_encode($data));
